...
function gc-proxy() {
  ...
  PROJECTS=$(gcloud projects list --format='csv(project_id,name,project_number)' | sed 's/,/ - /g')
  PROJECT=$(echo $PROJECTS | gum filter | cut -d ' ' -f 1)
  
  # Get list of cloud run services and select one
  SERVICES=$(gcloud run services list --project $PROJECT --format='csv(SERVICE,URL)' | sed 's/,/ - /g')
